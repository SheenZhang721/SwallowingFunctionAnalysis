...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W, mask):
        ...
        if self.sr_ratio > 1:
            if mask is None:
                # global
                q1 = self.q1(x).reshape(B, N, self.num_heads//2, C // self.num_heads).permute(0, 2, 1, 3)  # B H//2 N C//H
                ...
                # use mask to fuse
                mask_1, mask_2 = mask
                mask_sort1, mask_sort_index1 = torch.sort(mask_1, dim=1)  # horizontal
                mask_sort2, mask_sort_index2 = torch.sort(mask_2, dim=1)  # vertical
                if self.sr_ratio == 8:
                ...
                if self.sr_ratio==4 or self.sr_ratio==8:
                    ...
                    p2 = torch.gather(x, 1, mask_sort_index1[:, H * W // 4:H * W // 4 * 3].unsqueeze(-1).repeat(1, 1, C))  # B N//2 C
                    p3 = torch.gather(x, 1, mask_sort_index1[:, H * W // 4 * 3:].unsqueeze(-1).repeat(1, 1, C))  # B N//4 C
    ...
    def __init__(self, img_size=224, in_channel=1, embed_dims=64, num_heads=2,
        ...
        if patch_embed_downscale == 4:
            ...
            self.patch_embed = nn.Sequential(
                ...
                nn.GELU(),
                Conv2d_BN(embed_dims, embed_dims, 3, 2, 1),
            )
...
class SGFormerUnet(nn.Module):
    def __init__(self, n_channels=1, n_classes=1):
        ...
        # Question here
        self.maxpool = nn.MaxPool2d(2)
        self.SGe1 = SGFormerEncoder(img_size=224, in_channel=1, embed_dims=64, num_heads=2, depths=2, sr_ratio=4)
        self.SGe2 = SGFormerEncoder(img_size=112, in_channel=64, embed_dims=128, num_heads=4, depths=4, sr_ratio=4)
        self.SGe3 = SGFormerEncoder(img_size=56, in_channel=128, embed_dims=256, num_heads=8, depths=4, sr_ratio=4)
        self.SGe4 = SGFormerEncoder(img_size=28, in_channel=256, embed_dims=512, num_heads=16, depths=1, sr_ratio=4)
        self.SGe5 = SGFormerEncoder(img_size=14, in_channel=512, embed_dims=512, num_heads=16, depths=1, sr_ratio=1, patch_embed_downscale=2)

        self.up = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True)

        # self.conv120 = DecoderConv(16 * self.number, 8 * self.number)
        ...
        #     device='cuda',
        # )
        # self.conv13 = DecoderConv(24 * self.number, 8 * self.number)
        #
        # self.conv140 = DecoderConv(12 * self.number, 4 * self.number)
        ...
        #     device='cuda',
        # )
        # self.conv15 = DecoderConv(12 * self.number, 4 * self.number)

        # self.conv160 = DecoderConv(4 * self.number, 2 * self.number)
        ...
        #     device='cuda',
        # )
        # self.conv17 = DecoderConv(4 * self.number, 2 * self.number)
        #
        # self.conv180 = DecoderConv(3 * self.number, self.number)
        # # self.conv18x = DSConv_pro(
        # #     3 * self.number,
        # #     self.number,
        # #     self.kernel_size,
        # #     self.extend_scope,
        ...
        #     device='cuda',
        # )
        # self.conv19 = DecoderConv(2 * self.number, self.number)

        self.decoder4 = UpBlock(1024, 256, 2)
        self.decoder3 = UpBlock(512, 128, 2)
        self.decoder2 = UpBlock(256, 64, 2)
        self.decoder1 = UpBlock(128, 64, 2)
        self.outc = nn.Conv2d(64, self.n_classes, kernel_size=(1,1))

        # in_channels = 64
        self.inc = ConvBatchNorm(n_channels, in_channels)
        self.CNNdown2 = DownBlock(in_channels, in_channels*2, nb_Conv=2)
        self.CNNdown3 = DownBlock(in_channels*2, in_channels*4, nb_Conv=2)
        self.CNNdown4 = DownBlock(in_channels*4, in_channels*8, nb_Conv=2)
        self.CNNdown5 = DownBlock(in_channels*8, in_channels*8, nb_Conv=2)
        ...
        self.ViT_gate5 = AttentionGate(in_channels*8, in_channels*8, in_channels*8)
        ...
        self.CNN_gate5 = AttentionGate(in_channels*8, in_channels*8, in_channels*8)
    ...
    def forward(self, x):
        # Question here
        x = x.float()
        x1 = self.inc(x) # 224x224x64
        x1_t = self.SGe1(x) # 224x224x64
        # xt1 = self.TT1(x1+x1_t)

        x2 = self.CNNdown2(self.ViT_gate1(x1_t, x1)) # 112x112x128
        # xt2 = self.TT2(x2)
        x2_t = self.SGe2(self.maxpool(self.CNN_gate1(x1, x1_t))) # 112x112x128

        x3 = self.CNNdown3(self.ViT_gate2(x2_t, x2))  # 56x56x256
        # xt3 = self.TT3(x3)
        x3_t = self.SGe3(self.maxpool(self.CNN_gate2(x2, x2_t)))  # 56x56x256

        x4 = self.CNNdown4(self.ViT_gate3(x3_t, x3))  # 28x28x512
        # xt4 = self.TT4(x4)
        x4_t = self.SGe4(self.maxpool(self.CNN_gate3(x3, x3_t)))  # 28x28x512

        x5 = self.CNNdown5(self.ViT_gate4(x4_t, x4))  # 14x14x512  # todo: add temporal transformer
        x5_t = self.SGe5(self.maxpool(self.CNN_gate4(x4, x4_t)))  # 14x14x512

        x5_ = self.ViT_gate5(x5_t, x5)  # 14x14x512
        x5_t_ = self.CNN_gate5(x5, x5_t)  # 14x14x512
        x5 = x5_ + x5_t_

        x = self.decoder4(x5, x4+x4_t)  # 28x28x512
        x = self.decoder3(x,  x3+x3_t)  # 56x56x256
        x = self.decoder2(x, x2+x2_t)  # 112x112x128
        x = self.decoder1(x, x1+x1_t)  # 224x224x64

        # x = self.up(x5)  # 28x28x512
        # x_120 = self.conv120(torch.cat([x, x4+x4_t], dim=1))  # 28x28x1024 -> 28x28x512
        # x_12x = self.conv12x(torch.cat([x, x4+x4_t], dim=1))  # 28x28x1024 -> 28x28x512
        # x_12y = self.conv12y(torch.cat([x, x4+x4_t], dim=1))  # 28x28x1024 -> 28x28x512
        # x = self.conv13(torch.cat([x_120, x_12x, x_12y], dim=1)) # 28x28x1536 -> 28x28x512
        #
        # x = self.up(x)  # 56x56x256
        # x_140 = self.conv140(torch.cat([x, x3+x3_t], dim=1))  # 56x56x512 -> 56x56x256
        # x_14x = self.conv14x(torch.cat([x, x3+x3_t], dim=1))
        # x_14y = self.conv14y(torch.cat([x, x3+x3_t], dim=1))
        # x = self.conv15(torch.cat([x_140, x_14x, x_14y], dim=1))

        # x = self.up(x)  # 112x112x128
        # x_160 = self.conv160(torch.cat([x, x2+x2_t], dim=1))  # 112x112x256 -> 112x112x128
        # x_16x = self.conv16x(torch.cat([x, x2+x2_t], dim=1))
        # x_16y = self.conv16y(torch.cat([x, x2+x2_t], dim=1))
        # x = self.conv17(torch.cat([x_160, x_16y], dim=1))

        # x = self.up(x)  # 224x224x64
        # x_180 = self.conv180(torch.cat([x, x1+x1_t], dim=1))
        # x_18x = self.conv18x(torch.cat([x, x1+x1_t], dim=1))
        # x_18y = self.conv18y(torch.cat([x, x1+x1_t], dim=1))
        # x = self.conv19(torch.cat([x_180, x_18y], dim=1))
